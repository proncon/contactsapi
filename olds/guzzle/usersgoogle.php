...
if ($argv[1]=="-Delete")
	if (isset($argv[2]) && isset($argv[3]))
	{
		if ($argv[2]=="User")
		{
			...
			$response = $client->delete('http://localhost:9992/slimserver.php/paulo/37/contacts/'.$argv[3].'/',  [
	            'headers' => $headers,
	        ]);
		...
		if ($argv[2]=="Group")
		{
			echo "Delete Group";
			$response = new Response();
			$response = $client->delete('http://localhost:9992/slimserver.php/paulo/37/groups/'.$argv[3].'/',  [
	            'headers' => $headers,
	        ]);
	    }
...
foreach ($groupsArrAPI as $groupItem)
{
	...
	$url="http://localhost:9992/slimserver.php/paulo/37/groups/";
